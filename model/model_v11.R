...
## However, it can be used to derive other more relavant information like
...
## Get the names of columns that are of the character data type.
...
TrainingSet <- df_wash16[TrainingIndex,] # Training data
TestingSet <- df_wash16[-TrainingIndex,] # Test data
...
## The cost of a false positive (incorrectly predicting positive) is 5.
## The cost of a true negative (correctly predicting negative) is 0.
## The cost of a false negative (incorrectly predicting negative) is 1.
## This cost matrix will make the model more conservative about predicting
## positives, as the cost of false positives is high. You can adjust the values
## according to your specific needs. Remember, the higher the cost, the more the
## model will try to avoid that type of error.
costMatrix <- matrix(c(0, 1, 10, 0), nrow = 2)
...
model <- randomForest(loan_status ~ ., data = TrainSet, ntree = 500, mtry = 3, importance = TRUE,  cutoff = c(0.70, 0.30), parms = list(loss=costMatrix))
