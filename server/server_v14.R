...
shinyServer(function(input, output, session) {
  ...
  datasetInput <- reactive({
    ...
    # char_cols <- names(ui_input)[sapply(ui_input, is.character)]
    # print(char_cols)
    
    ## Convert character columns to factors, so they can work with the random forest.
    # ui_input[char_cols] <- lapply(ui_input[char_cols], as.factor)
    ...
    ## Define an empty row and column for the dependent
  ...
  output$vecImpPlot <- renderPlot({ 
    ...
    ## Margin parameter = 2 refers to sorting of the column values.
  ...
  output$analysis <- renderText({ 
    ...
    if ((plot_input$column) == "Approved") { 
      ...
      A larger value indicates that splitting the data based on that feature
