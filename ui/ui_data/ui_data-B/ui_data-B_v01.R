...
ui_dataframe_B <- data.frame(matrix(ncol = 3, nrow = (length(TrainSet))))

## Name the rows in numeric order for the new data frame.
colnames(ui_dataframe_B) <- c("column_name",
                            "stylized_name",
                            "menu_choices")
...
while (input_row_n < length(TrainSet)){
  input_row_n <- (input_row_n+1)
  new_row_n <- (new_row_n+1)
  
  column_name <- names(TrainSet)[input_row_n]

  ## If the column is not numeric, continue.
  if (!is.numeric(TrainSet[[column_name]])) {
    
    ## Assign the column_name value to the (first) input_row_n row of the column "column_name".
    ui_dataframe_B[new_row_n,"column_name"] <- column_name
    ...
    ## (input_row_n.e. Applicant Race Name 1, Co Applicant Race Name 1)
    is_digit <- grepl("\\d$", stylized_name)
    
    ## Remove the last character and the space before it if it is a digit.
    ## (input_row_n.e. Applicant Race Name, Co Applicant Race Name)
    ...
    ## (input_row_n.e. Loan Amount 000s, Applicant Income 000s)
    stylized_name <- str_replace(stylized_name, "000s$", "(Thousands)")
    
    ## Replace "Co " with "Co-" at the start of the string.
    ## (input_row_n.e. Co Applicant Sex Name, Co Applicant Ethnicity Name)
    stylized_name <- str_replace(stylized_name, "^Co ", "Co-")
    
    ## Assign the stylized name value to the input_row_n row of the column "stylized_name".
    ui_dataframe_B[new_row_n,"stylized_name"] <- stylized_name
    # print(ui_dataframe)
    ...
    ui_dataframe_B[new_row_n,"menu_choices"] <- list(menu_choices)
  }
    ...
    ## Remove the last row of the new data frame.
...
write.table(ui_dataframe_B, file = "ui_data_B.csv", sep = ",",
