...
while (i < length(TrainSet)){
  ...
  ## Assign the column_name value to the (first) i row of the column "column_name".
  ...
  ## (i.e. Applicant Race Name 1, Co Applicant Race Name 1)
  is_digit <- grepl("\\d$", stylized_name)
  
  ## Remove the last character and the space before it if it is a digit.
  ## (i.e. Applicant Race Name, Co Applicant Race Name)
  ...
  ## (i.e. Loan Amount 000s, Applicant Income 000s)
  stylized_name <- str_replace(stylized_name, "000s$", "(Thousands)")

  ## Replace "Co " with "Co-" at the start of the string.
  ## (i.e. Co Applicant Sex Name, Co Applicant Ethnicity Name)
  stylized_name <- str_replace(stylized_name, "^Co ", "Co-")
  
  ## Assign the stylized name value to the i row of the column "stylized_name".
  ...
  if (is.numeric(TrainSet[[column_name]])) {
    ...
    ## Append step_size value to the i row of "step_size" column.
...
write.table(ui_dataframe, file = "ui_data_df.csv", sep = ",",
