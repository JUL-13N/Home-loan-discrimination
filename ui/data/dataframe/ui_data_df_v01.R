...
# row.names(ui_dataframe)[1] <- "stylized_name"
# row.names(ui_dataframe)[2] <- "numeric_slider"
# row.names(ui_dataframe)[3] <- "mean_column"
# row.names(ui_dataframe)[4] <- "min_val"
# row.names(ui_dataframe)[5] <- "max_val"
# row.names(ui_dataframe)[6] <- "step_size"
# row.names(ui_dataframe)[7] <- "menu_choices"
...
lapply(seq_along(TrainSet), function(i) {
  ...
  if (is.numeric(TrainSet[[column_name]])) {
    ...
    ui_dataframe[(row.names(df))["mean_column"],i] <- mean_column
    ...
    ui_dataframe[(row.names(df))["min_val"],i] <- min_val
    
    ## Append max_val value to the "max_val" row for the i column.
    ui_dataframe[(row.names(df))["max_val"],i] <- max_val
    ...
    ui_dataframe[(row.names(df))["step_size"],i] <- step_size
  } 
  
  else {
    ## If the column is not numeric, assign FALSE to the numeric slider row variable.
    ui_dataframe[(row.names(df))["numeric_slider"],i] <- FALSE
    
    ## Append empty values to the rows that refer to the numeric columns.
    ui_dataframe[(row.names(df))["mean_column"],i] <- NA
    ui_dataframe[(row.names(df))["min_val"],i] <- NA
    ui_dataframe[(row.names(df))["max_val"],i] <- NA
    ui_dataframe[(row.names(df))["step_size"],i] <- NA
    ...
    ui_dataframe[(row.names(df))["menu_choices"],i] <- menu_choices
  }
})

## Write the data frame to a csv file.
write.table(ui_dataframe, file = "ui_data.csv", sep = ",", encoding = "UTF-8", row.names = TRUE, col.names = TRUE)
